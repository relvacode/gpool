...
func (FIFOScheduler) Evaluate(s []*WorkState) (int, time.Duration, bool) {
	return 0, AsSoonAsPossible, true
}

func (FIFOScheduler) Unload(*WorkState) {}
...
func (LIFOScheduler) Evaluate(jobs []*WorkState) (int, time.Duration, bool) {
	return len(jobs) - 1, AsSoonAsPossible, true
}

func (LIFOScheduler) Unload(*WorkState) {}
...
func (sch RuleScheduler) Unload(j *WorkState) {
	for _, r := range sch {
		r.Unload(j)
	}
}

func (sch RuleScheduler) Evaluate(jobs []*WorkState) (int, time.Duration, bool) {
